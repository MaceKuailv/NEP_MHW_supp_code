...
def give_me_ecco_heat(path,wallt_path,time_mean_vel = False):
...
def give_me_ecco_heat_seas(path,wallt_path,time_mean_vel = False):
    adv = xr.open_zarr(path+'adv.zarr')
    dif_h = xr.open_zarr(path+'dif_h.zarr')
    dif_v = xr.open_zarr(path+'dif_v.zarr')
    forch = xr.open_zarr(path+'forcH.zarr')
    ...
    ds['divuty'] = seas.divujty+seas.divuyty
